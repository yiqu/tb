...
- `useTableColumnDisplayToggle(tableId)` — `showColumn` / `hideColumn` / `toggleColumnDisplay`. Showing a
  column moves it to the last position of the table's ordering.
- `useTableColumnReorder(tableId)` — persists a drag and drop reorder of the visible columns without
  colliding with the ordinals held by hidden columns.
