...
export default function UpcomingBillsTablePaginationPageCountSelectList({
  paginationPromise,
}: {
  ...
  const handleOnChangePageSize = async (value: string) => {
    await toast.promise(
      upsertPaginationData({
        id: pagination?.id ?? '',
        pageId: SORT_DATA_PAGE_IDS.upcoming,
        pageSize: Number.parseInt(value),
      }),
      {
        loading: 'Updating...',
        success: `Updated upcoming bills page size: ${value}`,
        error: `Failed to update upcoming bills page size to ${value}`,
      },
    );
  };
