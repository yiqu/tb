...
export default function SubscriptionsTablePaginationPageCountSelectList({
  paginationPromise,
}: {
  ...
  const handleOnChangePageSize = async (value: string) => {
    await toast.promise(
      ...
      {
        loading: 'Updating...',
        success: `Updated subscriptions page size: ${value}`,
        error: `Failed to update subscriptions page size to ${value}`,
      },
    );
  };
