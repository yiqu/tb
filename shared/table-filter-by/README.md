...
export const TABLE_FILTER_BY_COLUMNS: Partial<Record<AppColumnId, TableFilterByColumnConfig>> = {
  ...
  frequency: { searchParamKey: 'frequencyText', placeholder: 'e.g. monthly' },
  // ...
};
```
...
the same page. `frequency` is the only such case today: that param is the bills page's frequency
multi select (a comma separated list of exact values, also written by the cell hover "add to
filter" button), and this free text `contains` filter is a different thing, so it uses
`frequencyText`.
